...
#import <qiYuSDK/QiYuSDKManager.h>
...
#pragma mark - 获取游戏对接参数plist文件的字典数据
    ...
    //初始化通知回调
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(initNotice:) name:@"qiYuSDKInit" object:nil];
    
    
    [QiYuSDKManager initializeQiYuSDKWithAppId:self.app_id appKey:self.app_key appScheme:self.client_key direction:[self.client_id intValue]];
...
-(void)channelsLogin{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loginNotice:) name:@"qiYuSDKLogin" object:nil];
    
    [QiYuSDKManager addLoginView:[self topController]];
...
- (void)checkRealName:(id)sender
{
    ...
    [QiYuSDKManager checkRealNameVerify];
}
...
-(void)channelUploadRole{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sendRoleNotice:) name:@"qiYuSDKSendRole" object:nil];
    
    //上报
    QYGameRoleInfo* roleInfo = [[QYGameRoleInfo alloc] init];
    ...
    [QiYuSDKManager sendQYGameRoleInfo:roleInfo];
    
}
#pragma mark - 渠道登出
-(void)channelsLogout{
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(theLoginOut) name:@"qiYuSDKLoginOut" object:nil];
...
-(void)channePay{
    ...
    */
    [QiYuSDKManager PayWithMoney:self.key_productPrice ProductId:self.key_productId ProductName:self.key_productName ProductDesc:self.key_productdesc RoleId:self.key_roleId RoleName:self.key_roleName ExtInfo:self.key_ext ServerId:self.key_serverId ServerName:self.key_serverName];
...
- (void) loginNotice:(NSNotification *)notification
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(realNameVerifyNotice:) name:@"qiYuSDKRealNameVerifySucc" object:nil];
